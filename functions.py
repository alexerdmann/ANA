...
### REST OF ASSIGNCELLS_INITIALIZATION(rough)
		...
		# """
		# COMPLEXITY IS EPOCHS*BATCHSIZE*#(PARADIGMS)*X
		# WHERE X IS COMPLEXITY OF COMPARING 2 PARADIGMS TO UPDATE EXP WT GRAPH AND (RE)ASSIGN CELLS
		# I.E., COMPLEXITY WILL ONLY EXPLODE IF X IS INEFFICIENT
		# """

		# ### for a prespecified number of epochs
		# for epoch in range(epochs):

		# 	remaining_keys = list(self.data)
		# 	random.shuffle(remaining_keys)

		# 	## Assign cell memberships to every paradigm
		# 	while len(remaining_keys) > 0:
		# 		lemma_new = remaining_keys.pop()
		# 		# extracting a sample of previously assigned paradigms for comparison
		# 		batch = random.sample(list(self.array), min(batchsize, len(self.array)))
		# 		if start:
		# 			self.assignParadigm_random(lemma_new)
		# 			start = False
		# 		else:
		# 			losses = {}
		# 			# for each binary paradigm comparison, based on pairwise cellmate likelihoods:
		# 			for lemma_old in batch:
		# 				# update exponent weight graph, get goodness of fit for sampled lemma cells
		# 				pass

		# 		# assign the new lemma cell memberships and consider re-assigning batch sampled lemmas
		# 		assignParadigm_initialization(self, lemma, batch, exponentWeightGraph, losses)


	# def assignParadigm_initialization(self, lemma, batch, exponentWeightGraph, losses):

	# 	### assign cell membership for lemma and consider reassigning batch sampled lemmas
	# 	self.array[lemma] = {}  # ...
	# 	for wf in self.data[lemma]
	# 		listCord = [] # determine the best coordinate choice and save as a list
